...
| `folder_zipper.py`                  | 批量压缩当前目录下的模块文件夹，生成与文件夹同名的ZIP压缩包（保留原目录结构） | 模块文件打包归档、备份       |
...
### 场景 6：模块文件夹批量打包归档
1. 确保`folder_zipper.py`放在根工作目录（与其他脚本同目录）；
2. 运行脚本：
```bash
python folder_zipper.py
```
3. 结果：根目录下生成各模块文件夹对应的同名 ZIP 压缩包（如`电机驱动模块-V1.0.zip`），保留原文件夹内的文件结构，且自动跳过已存在的压缩包。
